  ...
  chrome.windows.getCurrent(function(wnd) {
    chrome.tabs.getSelected(null, function(tab) {
      chrome.windows.create({
        url: 'https://oauth.vk.com/authorize?client_id=6483671&scope=docs,messages&redirect_uri=' + redirect_uri + '&display=page&response_type=token&v=5.75',
        tabId: tab.id,
        focused: true,
        type: 'popup',
...
function api(method, params, callback) {
  var arr = ['v=5.75', 'access_token=' + opts.accessToken];
  for (var k in params) {
